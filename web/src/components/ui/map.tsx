...
function MarkerTooltip({
  ...
  return createPortal(
    <div
      className={cn(
        "bg-popover text-popover-foreground border border-border pointer-events-none rounded-md px-2 py-1.5 text-xs text-balance shadow-md",
        "animate-in fade-in-0 zoom-in-95 duration-200 ease-out",
        className,
      )}
    >
...
function ControlGroup({ children }: { children: React.ReactNode }) {
  return (
    <div className="border-border/60 bg-background/85 dark:bg-card/85 backdrop-blur-md [&>button:not(:last-child)]:border-border/50 flex flex-col overflow-hidden rounded-lg border shadow-md [&>button:not(:last-child)]:border-b">
      {children}
    </div>
  );
}
